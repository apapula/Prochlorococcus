...
'''
This script looks for putative transfers from a cluster of cells (that is closely-related at the whole genome level, such as Kashtan's CN2 C1-C5 as well as a few small clusters found in the Berube data set) into other SAGs in HLII
'''
...
def make_plots(clustering_genes,berube_ribotypes,berube_ribogenes,found_genes,found_gene_cells,totncells):
    ...
    if 1:#sample striation
        ...
        plt.xlabel('Berube Sample Number',fontsize=25)
        ax.tick_params(axis='both', which='major', labelsize=25)
        plt.ylabel('C1-C5 Genes Per SAG',fontsize=25)
        plt.title('Number of C1-C5 genes as a function of sample',fontsize=25)
        plt.show()
        ...
        for j in range(len(cluster_indexlist)):
            if j!=i:
                ...
                if intermin=='-':
                    ...
                    return 0# only 1 ribotype is covered.
...
if 0:
    ...
    check_for_Berube_SAGs_Clustered_with_Berube_clusters(infilematrices,tfna,cluster_cells,celllist_berube,cluster_groupname,HLII_NoClose)
